...
def colorize_image(file, extension):
    ...
    processed_image = np.concatenate((L[:, :, np.newaxis], ab), axis=2)
    processed_image = cv2.cvtColor(processed_image, cv2.COLOR_LAB2BGR)
    processed_image = (255 * processed_image).astype("uint8")

    cv2.imwrite("processed_image." + extension, processed_image)
